...
function ModeSwitcher({
  ...
  const activeHidden = !atHome && overflow.some((m) => m.mode === mode);
        ...
        return (
          <button
            ...
            className={`flex flex-1 min-w-0 items-center justify-center gap-1.5 rounded px-2 py-1.5 text-sm ${
              !atHome && mode === m.mode ? "bg-accent/15 text-accent" : "hover:bg-surface text-muted"
            }`}
            onClick={() => onModeChange(m.mode)}
          >
            {m.icon} <span className="truncate">{m.label}</span>
            {unread > 0 && (atHome || mode !== m.mode) && <UnreadBadge count={unread} />}
          </button>
        );
      })}
      {overflow.length > 0 && (
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-2 px-3 py-2 text-left text-sm ${
                      !atHome && mode === m.mode ? "text-accent" : "hover:bg-surface"
                    }`}
                    onClick={() => {
                      setOverflowOpen(false);
                      onModeChange(m.mode);
