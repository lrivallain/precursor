...
async def ensure_schedule_root(session: AsyncSession) -> Topic:
    ...
    root = Topic(
        ...
        slug=await allocate_unique_slug(session, SCHEDULE_ROOT_SLUG, Topic),
        kind="schedule_root",
    )
    session.add(root)
...
async def create_schedule(
    ...
    topic = Topic(
        title=payload.title,
        slug=await allocate_unique_slug(session, base, Topic),
        kind="scheduled",
        parent_id=root.id,
    )
    session.add(topic)
...
async def run_now(topic_id: int, session: AsyncSession = Depends(get_session)) -> TopicSchedule:
    ...
    # run bypasses the prompt's /guard emptiness gate — an explicit "Run now"
    # should start even when the guard would otherwise report "no work".
