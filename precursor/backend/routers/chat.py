...
def _extract_pdf_text(data: bytes) -> str:
    try:
        ...
        for page in reader.pages:
            ...
            if txt:
                chunks.append(txt)
            if sum(len(s) for s in chunks) >= _MAX_ATTACHMENT_CONTEXT_CHARS:
                break
        parsed = "\n".join(chunks).strip()
        if parsed:
            return parsed
    ...
    for raw in re.findall(rb"\(([^()]*)\)\s*T[Jj]", data):
        ...
        if sum(len(s) for s in snippets) >= _MAX_ATTACHMENT_CONTEXT_CHARS:
            break
    return "\n".join(snippets)
            ...
            for node in root.iter():
                if node.tag.endswith("}t") and node.text:
                    ...
                    if txt:
                        chunks.append(txt)
                        total += len(txt)
                if total >= _MAX_ATTACHMENT_CONTEXT_CHARS:
                    return "\n".join(chunks)
    return "\n".join(chunks)
...
def _attachments_to_text_context(atts: list[Attachment]) -> str:
    if not atts:
        return ""
    ...
    for att in atts:
        ...
        if text:
            trimmed = text[:_MAX_ATTACHMENT_CONTEXT_CHARS]
...
def _hydrate_history(rows: list[Message]) -> list[ChatMessage]:
    ...
    while i < len(rows):
        ...
        if role == "user" and m.attachments:
            ...
            image_urls = _attachments_to_image_urls(
                [att for att in user_attachments if _is_image_attachment(att)]
            )
            non_image_context = _attachments_to_text_context(
                [att for att in user_attachments if not _is_image_attachment(att)]
            )
