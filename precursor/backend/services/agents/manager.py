...
protocol in ``directives``, MCP server scoping in ``mcp_scope`` and SDK shutdown
log filtering in ``sdk_logging``.
"""
...
# The pure helpers moved to leaf modules; listing them here keeps existing
# ``from …agents.manager import …`` callers working (and explicit for mypy).
__all__ = [
    "MCP_SCOPE_MAX_LEN",
    "RESULT_SUMMARY_CAP",
    "AgentManager",
