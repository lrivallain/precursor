...
| `live` | [Live sessions](/features/live-sessions) — transcript segments, insights, notes, summary, and attendees. `GET /{id}/transcripts` lists the linked Teams meeting's transcription sessions (id + `created_at`/`ended_at`, oldest first, fail-closed with a `detail`), and `POST /{id}/summary/from-transcript` takes an optional `transcript_ids` body to summarize the chosen session(s) — several are stitched in chronological order; omitted, the most recent is used. |
