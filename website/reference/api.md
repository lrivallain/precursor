...
| `topic-summary` | The topic's editable [status brief](/features/topic-summary), as a sub-resource: `GET`/`PUT`/`DELETE /api/topics/{id}/topic-summary` (read — `null` when the topic has none — save an edit, drop it), `POST …/visibility`, `POST …/generate` (refresh from the conversation, notes and attachments), `POST …/resolve` (merge a proposal, taking only the accepted change indices) and `POST …/items` (append a `todo` or `important` line). Distinct from `POST /api/topics/{id}/summary`, which summarises the *linked GitHub issue*. |
